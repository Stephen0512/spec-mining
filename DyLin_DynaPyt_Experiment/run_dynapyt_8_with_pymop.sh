...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_DynaPyt_8_with_pymop"
...
# Special handling for some repositories
if [ "${DEVELOPER_ID}-${TESTING_REPO_NAME}_${target_sha}" == "SeleniumHQ-selenium_97d56d04e1b4ab4f8e527f8849b777c1e91d13f7" ]; then
    /usr/bin/time -v timeout -k 9 3400 pytest py/ --continue-on-collection-errors -p no:sugar &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
else
    /usr/bin/time -v timeout -k 9 3400 pytest --continue-on-collection-errors -p no:sugar &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
fi
