...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_DynaPyt_Libs_with_pymop"
...
SOURCE_DIR="$PWD/../Specs_libs_with_PyMOP/DynaPyt"
...
cp "$PWD/../Specs_libs_with_PyMOP/dynapyt_analyses.txt" "$TMPDIR/dynapyt_analyses-$DYNAPYT_SESSION_ID.txt"
...
# Special handling for some repositories
if [ "${DEVELOPER_ID}-${TESTING_REPO_NAME}_${target_sha}" == "SeleniumHQ-selenium_97d56d04e1b4ab4f8e527f8849b777c1e91d13f7" ]; then
    /usr/bin/time -v timeout -k 9 7000 pytest py/ --continue-on-collection-errors -p no:sugar &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
else
    /usr/bin/time -v timeout -k 9 7000 pytest --continue-on-collection-errors -p no:sugar &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
fi
