...
CLONE_DIR="${DEVELOPER_ID}-${TESTING_REPO_NAME}_PyMOP_A_Libs"
...
# Special handling for some repositories
if [ "${DEVELOPER_ID}-${TESTING_REPO_NAME}_${target_sha}" == "SeleniumHQ-selenium_97d56d04e1b4ab4f8e527f8849b777c1e91d13f7" ]; then
    /usr/bin/time -v timeout -k 9 12000 bash -c 'PYMOP_SPEC_FOLDER="$PWD"/../pymop/specs-new PYMOP_ALGO=A PYMOP_INSTRUMENTATION_STRATEGY=ast PYMOP_INSTRUMENT_SITE_PACKAGES=True PYMOP_STATISTICS=yes PYMOP_STATISTICS_FILE=A.json PYTHONPATH="$PWD"/../pymop/pythonmop/pymop-startup-helper/ pytest py/ --continue-on-collection-errors -p no:sugar' &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
else
    /usr/bin/time -v timeout -k 9 12000 bash -c 'PYMOP_SPEC_FOLDER="$PWD"/../pymop/specs-new PYMOP_ALGO=A PYMOP_INSTRUMENTATION_STRATEGY=ast PYMOP_INSTRUMENT_SITE_PACKAGES=True PYMOP_STATISTICS=yes PYMOP_STATISTICS_FILE=A.json PYTHONPATH="$PWD"/../pymop/pythonmop/pymop-startup-helper/ pytest --continue-on-collection-errors -p no:sugar' &> ${TESTING_REPO_NAME}_Output.txt
    exit_code=$?
fi
...
cp $TESTING_REPO_NAME/A-full.json $CLONE_DIR/A-full.json
cp $TESTING_REPO_NAME/A-time.json $CLONE_DIR/A-time.json
cp $TESTING_REPO_NAME/A-violations.json $CLONE_DIR/A-violations.json
